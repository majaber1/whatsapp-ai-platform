  ...
  try {
    ...
    for (const config of configs) {
      ...
      try {
        ...
        // Malformed or wrong-key row: continue checking other configs.
      }
    }
...
async function processWebhook(body: { entry?: WhatsAppWebhookEntry[] }) {
  ...
  for (const entry of body.entry) {
    for (const change of entry.changes) {
      ...
      // CR-001 invariant: resolve the provider connection first. Status
      // events and inbound messages are never routed by WAMID alone.
      if (!phoneNumberId) {
      ...
      if (configRows.length > 1) {
        console.error(
          ...
          phoneNumberId,
          '— provider event dropped. Resolve duplicates so each number maps to a single account.',
          'Account owners:',
          configRows.map(
      ...
      if (value.statuses) {
        for (const status of value.statuses) {
          await handleStatusUpdate(status, config.id, config.account_id)
        }
      }
...
async function processMessage(
  ...
  await flagBroadcastReplyIfAny(accountId, contactRecord.id, whatsappConfigId)
